...
def get_dataset_and_loader(cfg, device):
    ...
    if dataset.startswith('SVHN'):
        ...
        data_train = t.stack([x[0] for x in tv.datasets.SVHN(download=True, root='data/svhn', transform=transform)]).to(device)
        data_test = t.stack([x[0] for x in tv.datasets.SVHN(split='test', download=True, root='data/svhn', transform=transform)]).to(device)
        data_valid= t.stack([x[0] for x in tv.datasets.SVHN(split='test', download=True, root='data/svhn', transform=transform)]).to(device)
    elif dataset == "MNIST":
        transform = tfm.Compose([tfm.Resize(cfg["img_size"]), tfm.ToTensor(), tfm.Normalize((0.5), (0.5),)])
        data_train = t.stack([x[0] for x in tv.datasets.MNIST(download=True, root='data/mnist', transform=transform)]).to(device)
        data_test = t.stack([x[0] for x in tv.datasets.MNIST(split='test', download=True, root='data/mnist', transform=transform)]).to(device)
