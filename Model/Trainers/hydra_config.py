...
class BaseTrainerConfig:
    ...
    nb_sample_fid_val: int = 5096
    nb_sample_partition_estimate_val: int = 1000

    # Test parameters :
    multiple_sample_test_SNIS: int = 1000
    multiple_sample_test: int = 1000
    nb_sample_fid_test: int = 30000
    nb_sample_partition_estimate_test: int = 10000
...
def store_base_trainer(cs: ConfigStore):
    cs.store(
        group="trainer",
        name="contrastive_divergence_trainer_base",
        node=TrainerContrastiveDivergence,
    )
    cs.store(
        group="trainer",
        name="contrastive_divergence_log_trick_trainer_base",
        node=TrainerContrastiveDivergenceLogTrick,
    )
    ...
    cs.store(
        group="trainer",
        name="snelbo_aggregate_trainer_base",
        node=TrainerSNELBOAggregatePosterior,
    )
