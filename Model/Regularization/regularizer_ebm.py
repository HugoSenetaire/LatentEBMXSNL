...
def regularization(ebm, x, x_gen, energy_data, energy_samples, cfg, logger, step):
        ...
        dic_loss = {}
        # Regularization
        if cfg.regularization.l2_grad is not None and cfg.regularization.l2_grad > 0:
            grad_norm = wgan_gradient_penalty(ebm, x, x_gen)
            dic_loss["l2_grad"] = cfg.regularization.l2_grad * grad_norm
            logger.log({"penalty/grad_norm": grad_norm},step=step)
            logger.log({"penalty/regularization_l2_grad": dic_loss["l2_grad"]},step=step)

        if cfg.regularization.l2_output is not None and cfg.regularization.l2_output > 0:
            l2_output = ((energy_data**2).mean() + (energy_samples**2).mean())
            dic_loss["loss_l2_output"] = cfg.regularization.l2_output * l2_output
        ...
        if cfg.regularization.l2_param is not None and cfg.regularization.l2_param > 0:
            penalty = 0.
            len_params = 0.
            ...
            dic_loss["loss_l2_param"] = cfg.regularization.l2_param * penalty
