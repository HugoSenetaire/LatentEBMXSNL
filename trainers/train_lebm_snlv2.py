...
from sampler import (
    sample_langevin_posterior,
    sample_langevin_prior,
    sample_p_0,
    sample_p_data,
)
...
class Trainer_LEBM_SNL2(AbstractTrainer):
    ...
    def train_step(self, x, step):
        ...
        log_partition_estimate = torch.logsumexp(
            -energy_proposal - proposal_z_proposal, 0
        ) - math.log(energy_proposal.shape[0])
        ...
        if self.cfg["regularize_entropy_posterior"] > 0:
            print("REGULARIZING")
            loss_total -= self.cfg["regularize_entropy_posterior"] * entropy_posterior

        dic_loss = regularization(
            self.E,
            z_q,
            z_e_0,
            energy_approximate,
        ...
        for key, item in dic_loss.items():
            loss_total += item
        loss_total.backward()
        grad_clipping_all_net(
            [self.E, self.G, self.Encoder],
            ["E", "G", "Encoder"],
            [
                self.optE,
                self.optG,
