...
from sampler import sample_langevin_posterior, sample_langevin_prior, sample_p_data
...
class AbstractTrainer:
    def __init__(
        ...
        self.logger = wandb.init(project="LatentEBM", config=cfg, dir=cfg["log_dir"])
    ...
    def train(self, train_data, val_data=None):
        for i in tqdm.tqdm(range(self.n_iter_pretrain + self.n_iter)):
            ...
            if i % self.cfg["save_image_every"] == 0:
                ...
                z_e_k = sample_langevin_prior(
                    z_e_0, self.E, self.cfg["K_0"], self.cfg["a_0"]
                )
                z_g_k = sample_langevin_posterior(
                    z_g_0,
                    x[:batch_save],
                    self.G,
                    self.E,
    ...
    def eval(self, val_data, step, name="val/"):
        with torch.no_grad():
            z_e_0 = self.base_dist.sample(
                (self.cfg["nb_sample_partition_estimate_val"], self.cfg["nz"], 1, 1)
            )
    ...
    def train_step_standard_elbo(self, x, step):
        ...
        KL_loss = 0.5 * (
            ...
            - log_var_q
            - 1
            + (log_var_q.exp() + mu_q.pow(2)) / log_var_p.exp()
        )
